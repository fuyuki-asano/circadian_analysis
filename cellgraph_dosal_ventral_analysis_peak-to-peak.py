...
#win
path_list = glob.glob('F:\\circadian_experiment\\python_scripts\\cellgraph\\dosal_ventral_data\\data_folder\\*.csv')


column = ['index', 'time_stamp', 'time_h', 'time_min', 'dosal_1', 'dosal_2', 'ventral_1', 'ventral_2']
...
emp = np.zeros(len(path_list)*4+1)
emp[:] = np.nan
emp = emp.reshape(-1,1)
...
#    df = pd.read_csv(path_list[i], names=column, skipfooter=2, engine='python')
    ...
    name_list = np.array([file_name for x in np.arange(1,5,1)]).reshape(-1,1)
    name = np.concatenate((name, name_list), axis=0)
    region_list = np.array(['dosal_1', 'dosal_2', 'ventral_1', 'ventral_2']).reshape(-1,1)
    region = np.concatenate((region, region_list), axis=0)
...
#data analysis 
    for ii in range(0,4,1):
        ...
        det_luc_zt = np.array(df.iloc[onset:offset, 8])
        sm_luc_zt = np.array(df.iloc[onset:offset, 9])
...
#                fig.savefig(file_name + '_' + region_list[ii,0] + '.png')
                plt.show()
...
with open ('cellgraph_dosal_ventral_analysis.csv', 'w', newline = '') as f:
    writer = csv.writer(f)
