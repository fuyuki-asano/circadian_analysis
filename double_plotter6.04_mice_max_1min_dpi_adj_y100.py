...
#    df.iloc[:,i] = column
    ...
    maxi = np.max(column) #全日程中の回転数の最大値を見つける
    ...
    v = u / maxi #最大値で割って、ratioにする
    v = np.round(v, 2)

    
    ref = np.concatenate((np.zeros((100, 60*6+1))\
                          , np.ones((100, 60*12))\
                          , np.zeros((100, 60*12))\
                          , np.ones((100, 60*12))\
                          , np.zeros((100, 60*6+1))), axis=1)
    ...
    for i in range(recdays + 1):
        w = np.array(v[i,:])
        w = (w*100).astype(np.int64)
        
        row = np.zeros((101, 1))
        
        for ii in range(len(w)):
            stac = np.concatenate((np.ones((100-w[ii], 1)), np.zeros((w[ii]+1, 1))), axis=0)
            row = np.concatenate((row, stac), axis=1)
            
        row = np.concatenate((row,  np.zeros((101, 1))), axis=1)
        actogram = np.concatenate((actogram, row), axis=0)
     
    name = str(Ch[ID]) + '_y100.tiff'    
